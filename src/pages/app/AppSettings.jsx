...
export function AppSettings() {
  ...
  const handleClearContacts = async () => {
    setClearing(true);
    try {
      await api("/api/app/people/contacts/all", { method: "DELETE", token });
      toastSuccess("Successfully cleared all contacts.");
      setShowClearConfirm(false);
    } catch (e) {
      toastFromError(e, "Failed to clear contacts");
    } finally {
      setClearing(false);
    }
  };
    ...
    <div className="space-y-6 p-4 sm:p-6">
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="space-y-6">
          ...
          <div className="rounded-2xl border border-slate-100 bg-white shadow-sm overflow-hidden">
            <div className="flex items-center gap-2 border-b border-slate-100 bg-slate-50 px-5 py-3">
              <LuSettings className="h-4 w-4 text-slate-500" strokeWidth={1.75} />
              <h3 className="text-sm font-bold text-slate-800">Data Management</h3>
            </div>
            <div className="p-5 space-y-4">
              <div>
                <h4 className="text-sm font-semibold text-slate-800">Clear Synced Contacts</h4>
                <p className="text-xs text-slate-500 mt-1">
                  Permanently delete all contacts synced to this workspace. This action cannot be undone.
                </p>
              </div>
              <button
                type="button"
                onClick={() => setShowClearConfirm(true)}
                className="w-full sm:w-auto rounded-lg bg-red-50 border border-red-200 hover:bg-red-100 px-4 py-2 text-xs font-semibold text-red-700 transition cursor-pointer"
              >
                Clear Contacts
              </button>
            </div>
      ...
      {/* Clear Contacts Confirmation Modal */}
      {showClearConfirm && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/55 p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm rounded-2xl bg-white p-6 shadow-xl border border-slate-100 animate-in fade-in zoom-in-95 duration-150">
            <h3 className="text-base font-bold text-slate-900">Clear all synced contacts?</h3>
            <p className="mt-2 text-xs text-slate-500 leading-relaxed">
              This will permanently delete all contacts synced to this workspace. Active chats or campaigns connected to these phone numbers may be disrupted.
            </p>
            <div className="mt-6 flex justify-end gap-3">
              <button
                ...
                className="rounded-lg border border-slate-200 bg-white px-3.5 py-2 text-xs font-semibold text-slate-700 hover:bg-slate-50 disabled:opacity-50 cursor-pointer"
              >
                Cancel
              </button>
              <button
                ...
                className="rounded-lg bg-red-600 px-3.5 py-2 text-xs font-semibold text-white hover:bg-red-700 disabled:opacity-50 cursor-pointer"
              >
